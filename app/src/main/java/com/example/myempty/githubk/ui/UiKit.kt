...
object UiKit {
    ...
    private fun sheetItemRow(c: Context, icon: String, title: String, subtitle: String?, colorRes: Int, onClick: () -> Unit): LinearLayout =
        LinearLayout(c).apply {
            ...
            setPadding(dp(c, 8), dp(c, 10), dp(c, 8), dp(c, 10))
            setOnClickListener { onClick() }
            addView(TextView(c).apply {
                text = icon.ifBlank { "\u2022" }.take(1).uppercase()
                textSize = 13f
                typeface = Typeface.DEFAULT_BOLD
                setTextColor(android.graphics.Color.WHITE)
                gravity = Gravity.CENTER
                background = rounded(c, themeColor(c, colorRes), 10)
            ...
            addView(TextView(c).apply { text = "\u203a"; textSize = 22f; setTextColor(themeColor(c, R.color.muted)) })
        }
