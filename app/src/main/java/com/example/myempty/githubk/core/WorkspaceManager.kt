...
class WorkspaceManager(private val c: Context) {
    ...
    fun sourceFiles(p: Project): List<File> =
        p.path.walkTopDown()
            .filter { it.isFile && it.extension.lowercase() in SOURCE_EXT }
            .take(300)
            .toList()
    ...
    fun packageSource(p: Project, includeBuild: Boolean = false): File {
        ...
        val excludedDirs = if (includeBuild)
            ...
            setOf("build", ".gradle", ".dart_tool", ".git", "gradle", "node_modules", ".idea", "cxx", ".cxx")
    ...
    fun searchCode(p: Project, q: String): String {
        if (q.isBlank()) return ""
        return sourceFiles(p).flatMap { f ->
            runCatching {
                f.readLines().mapIndexedNotNull { n, line ->
                    if (line.contains(q, true)) "${f.relativeTo(p.path).path}:${n + 1}: $line" else null
                }
