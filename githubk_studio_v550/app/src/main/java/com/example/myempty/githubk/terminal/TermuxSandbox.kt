...
object TermuxSandbox {
    ...
    /** 流式执行（回调每行输出），返回退出码；超时返回 124，被取消返回 130。
     *  cancelCheck 非空时每 200ms 检查一次：返回 true 立即整树强杀并返回 130。
     *  修复：旧实现把日志读取与超时等待串行化，构建长时间无输出时会一直卡在
     *  readLine，既超时不生效也感知不到「停止」；现改为独立读线程 + 主轮询循环。 */
    fun runStreaming(
        context: Context,
        prefixHost: File,
        command: String,
        ...
        return try {
            ...
            val p = pb.start()
            // 读线程：逐行回传日志；进程被强杀后流 EOF 自动退出
            val outThread = Thread {
                try {
                    p.inputStream.bufferedReader(Charsets.UTF_8).use { r ->
                        r.forEachLine { onOutput(it + "\n") }
                    }
                } catch (_: Throwable) {}
            }
            outThread.isDaemon = true
            outThread.start()
            val deadline = if (timeoutMin > 0) System.currentTimeMillis() + timeoutMin * 60000L else Long.MAX_VALUE
            var timedOut = false
            var stopped = false
            while (true) {
                if (!p.isAlive) break
                if (cancelCheck?.invoke() == true) { stopped = true; break }
                if (System.currentTimeMillis() > deadline) { timedOut = true; break }
                try { p.waitFor(200, TimeUnit.MILLISECONDS) } catch (_: Throwable) { break }
            }
            if (stopped || timedOut) {
                // 整树强杀：proot 是 ptrace 监督者，直接 kill 它可能遗留 guest 子进程，
                // 因此先按 /proc 后代枚举杀掉 proot 的所有 guest 后代，再杀 proot 本身。
                killTree(pidOf(p))
                runCatching { p.destroyForcibly() }
                try { p.waitFor(3, TimeUnit.SECONDS) } catch (_: Throwable) {}
                if (timedOut) return 124
                return 130
            }
            // 进程自然退出：确保读线程读完剩余输出
            try { outThread.join(3000) } catch (_: Throwable) {}
            p.exitValue()
        } catch (t: Throwable) {
    ...
    /** 反射读取 Java Process 的真实 pid（Android 的 java.lang.Process 无公开 pid()）。 */
    ...
    /** 枚举 /proc 找出 rootPid 的全部后代并强杀（自底向上），用于终止 proot 及其 guest 进程。 */
    private fun killTree(rootPid: Int) {
        val children = HashMap<Int, MutableList<Int>>()
        try {
            val proc = File("/proc")
            proc.listFiles()?.forEach { d ->
                val pid = d.name.toIntOrNull() ?: return@forEach
                runCatching {
                    val stat = File(d, "stat").readText(Charsets.UTF_8)
                    val close = stat.lastIndexOf(')')
                    if (close >= 0) {
                        val f = stat.substring(close + 1).trim().split(' ')
                        val ppid = f.getOrNull(1)?.toIntOrNull() ?: return@runCatching
                        if (ppid > 1) children.getOrPut(ppid) { ArrayList() }.add(pid)
                    }
                }
            }
        } catch (_: Throwable) {}
