...
class SettingsPage(private val host: PageHost) {
    ...
    private fun buildAiCard(): View {
        ...
        val card = UiKit.card(host.context, UiKit.vstack(host.context).apply {
            ...
            addView(UiKit.divider(host.context))
            addView(UiKit.spacer(host.context, 10))
            addView(UiKit.label(host.context, "本地 AI 推理（离线神经网络）· 分类 / 检测 / OCR 全在本机，图片不出设备", color = com.example.myempty.githubk.R.color.muted, size = 11.5f))
            addView(UiKit.spacer(host.context, 8))
            addView(UiKit.tonalButton(host.context, "打开本地 AI 推理") {
                host.pushPage(LocalAiPage(host).buildView())
            })
        })
        return card
    }
