...
class BuildEngine(private val onLog: (String) -> Unit = {}) {
    ...
    fun build(
        ...
        gradleHome: String = "",
        flutterBin: String = "",
        stream: ((String) -> Unit)? = null
    ): BuildResult {
        running = true
        cancelled = false
        val sb = StringBuilder()
        val log: (String) -> Unit = { sb.append(it); sb.append("\n"); onLog(it); stream?.invoke(it) }
    ...
    private fun launchNative(
        ...
        outThread.start()
        // 等待循环：每 400ms 检查一次取消标志，使「停止」即时生效
        while (true) {
            if (cancelled) { proc.destroyForcibly(); break }
            if (!proc.isAlive) break
            try { proc.waitFor(400, TimeUnit.MILLISECONDS) } catch (_: Throwable) { break }
        }
    ...
    private fun launchViaProot(
        ...
        onLog("$ $command")
        // 停止即时生效：cancelled 置位后 TermuxSandbox 整树强杀 proot + guest 构建进程
        return TermuxSandbox.runStreaming(ctx, rt.root, command, env, onLog, 60, false) { cancelled }
    }
